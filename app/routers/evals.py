...
from app.models import EvalRun, EvalStatus, EvalResult, Project, User
...
async def trigger_eval(eval: schemas.TriggerEval, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user_from_api_key)):
    result = await db.execute(select(Project).where(Project.id == eval.project_id))
...
async def get_result(eval_id: int, compared_to: int | None = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user_from_api_key)):
